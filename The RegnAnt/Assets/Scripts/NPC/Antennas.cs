...
public class Antennas : MonoBehaviour
{
    ...
    private void antennasMovement()
    {
        ...
        if (Physics.SphereCast(_headPosition.position, 0.5f, _headPosition.forward - _headPosition.right * 0.5f, out RaycastHit hitL, 1.1f))        
        ...
        if (Physics.SphereCast(_headPosition.position, 0.5f, _headPosition.forward + _headPosition.right * 0.5f, out RaycastHit hitR, 1.1f)) 
